...
impl Chain {
    ...
    pub async fn get_inflation_rate(&self) -> Result<f64, String> {
        ...
        let mut inflation = if ["evmos", "echelon"].contains(&chain_name) {
            ...
            inflation_rate.inflation_rate.parse::<f64>().map(|r| r / 100.0).map_err(|e| format!("Failed to parse float: {}", e))
           ...
           String::from_utf8(inflation_rate.inflation).unwrap().parse::<f64>().map(|r| r / 100.0).map_err(|e| format!("Faield to construct string: {}", e))
        }
