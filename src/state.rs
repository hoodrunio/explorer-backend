    ...
    pub async fn new() -> State {
        ...
        let chains = stream.then(|(name, config)| async move {
            let chain = match Chain::initialize(config, reqwest::Client::new(), DatabaseTR::new().await).await {
                ...
                Err(e) => { tracing::error!("Error initializing chain {name}: {e}"); None }
            };
            (name, chain)
        }).filter(|(name, chain)| futures::future::ready(chain.is_some())).map(|(name, chain)| (name, chain.unwrap())).collect::<HashMap<String, Chain>>().await;
    ...
    pub async fn subscribe_to_events(&self) {
        self.chains.clone().into_iter().for_each(|(name, chain)| {
            tokio::spawn(async move {
                match chain.subscribe_to_events().await {
